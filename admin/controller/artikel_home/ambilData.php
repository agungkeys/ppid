...
if(isset($_POST['page'])){
    ...
    $pagConfig = array('baseURL'=>'admin/controller/artikel_home/ambilData.php', 'totalRows'=>$rowCount, 'currentPage'=>$start, 'perPage'=>$limit, 'contentDiv'=>'posts_content');
    $pagination =  new Pagination($pagConfig);
    //Dapatkan Baris Data
    $query = $mysqli->query("SELECT artikel.IDARTIKEL, skpd.NAME, artikel.JUDULARTIKEL, artikel.ISIARTIKEL, artikel.IMG, artikel.USER, artikel.DATECREATE, artikel.IDSKPD FROM artikel INNER JOIN skpd ON artikel.IDSKPD = skpd.IDSKPD ORDER BY artikel.IDARTIKEL DESC LIMIT $start,$limit");
    if($query->num_rows > 0){ ?>
    <?php
